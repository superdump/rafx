...
impl RafxBufferDef {
    pub fn for_staging_buffer(size: usize, resource_type: RafxResourceType) -> RafxBufferDef {
        RafxBufferDef {
            size: size as u64,
            alignment: 0,
    ...
    pub fn for_staging_buffer_data<T: Copy>(data: &[T], resource_type: RafxResourceType) -> RafxBufferDef {
        Self::for_staging_buffer(
            rafx_base::memory::slice_size_in_bytes(data),
            resource_type
        )
    }
