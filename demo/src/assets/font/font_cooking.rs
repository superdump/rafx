...
pub fn create_font_texture(font_data: &[u8], size: f32, margin: u32) -> Option<FontTextureWithLookup> {
    ...
    for &c in &characters_to_include {
    ...
    let mut character_lookup = FnvHashMap::with_capacity_and_hasher(characters_to_include.len(), BuildHasherDefault::default());
    ...
    for (&c, (_, location)) in placement.packed_locations() {
        ...
        character_lookup.insert(c, FontTextureCharacterRect {
